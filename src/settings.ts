	...
	display(): void {
      ...
      .setDesc(
        createFragment((descEl) => {
          descEl.appendText("Extension list to include and exclude file during counting");
          descEl.appendChild(document.createElement("br"));
          descEl.appendText("Separated by line break and/or comma");
          descEl.appendChild(document.createElement("br"));
          descEl.appendText('enter "md" to include markdown file, "^mp4" to exclude mp4 file');
        }),
      )
      .addTextArea((text) => {
        text
          .setValue(
            this.plugin.settings.filterList.map((array) =>
              array.join(','),
            ).join('\n'),
          )
          .onChange((value) => {
